...
| Runtime health and verification | `./scripts/harness doctor --check-only`, `./scripts/harness verify` | `tests/installer/commands.test.mjs`, `tests/trio/authority-parity.test.mjs`, `tests/core/repo-workflow-acceptance.test.mjs` | always-on core + focused supplement + opt-in replay | Trio Core authority resolution and installer command health surfaces are covered, and the cross-workflow replay now exercises clean, degraded, lifecycle-sensitive, trust-boundary, and managed-target variants. |
