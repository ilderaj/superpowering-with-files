...
test('readHarnessHealth accepts generic Path labels and emphasized active task labels', async () => {
  const root = await createHarnessFixture();
  try {
    await writeState(root, {
      schemaVersion: 1,
    ...
    await writeFile(
      path.join(root, 'planning/active/task-a/task_plan.md'),
      '# Task plan\n\n## Companion plan\n\n- Path: docs/superpowers/plans/feature-plan.md\n'
    );
    await writeFile(path.join(root, 'planning/active/task-a/findings.md'), '# Findings\n');
    await writeFile(path.join(root, 'planning/active/task-a/progress.md'), '# Progress\n');
    await mkdir(path.join(root, 'docs/superpowers/plans'), { recursive: true });
...
test('readHarnessHealth classifies symlinked Copilot skill duplicates without failing display-only overlap', async (t) => {
  ...
  try {
    ...
    await mkdir(path.dirname(workspaceSkill), { recursive: true });
...
test('readHarnessHealth classifies symlinked Cursor skill duplicates without failing display-only overlap', async (t) => {
  const root = await createHarnessFixture();
  const home = path.join(root, 'home');
