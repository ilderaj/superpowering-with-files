...
def main():
    ...
    plan_dir = resolve_plan_dir(Path(project_path).resolve())
    planning_files = planning_file_map(Path(project_path).resolve())

    # Check if planning files exist (indicates active task)
    has_planning_files = any(
        Path(path_value).exists() for path_value in planning_files.values()
    )
    ...
    print(f"Active task dir: {plan_dir}")
    ...
    print(f"2. Read: {planning_files['task_plan.md']}, {planning_files['progress.md']}, {planning_files['findings.md']}")
