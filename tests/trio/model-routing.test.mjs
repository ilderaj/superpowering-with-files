...
import { resolveModelEffort, resolveHostOperation, packetDigestOf, routeTask, buildAssignmentPacket, freezeAssignmentPacket } from '../../harness/trio/core/routing.mjs';

function packet(capability = {}) {
...
const spawn = (extra = {}) => resolveHostOperation({ operation: 'spawn', assignmentPacket: packet(), parentEnvelope, childEnvelope, observation, ...extra });
...
for (const model of ['gpt-5.6-sol', 'gpt-5.6-terra']) {
  test(`frozen legacy Chief ${model} retains max when effort is absent`, () => {
    ...
    original.capability = { workRole: 'chief', requestedModel: model };
