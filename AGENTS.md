...
If a superpowers skill suggests saving a long-lived plan under `docs/superpowers/plans/`, treat that path as available only for a Deep-reasoning task companion plan. Create it only when the task is Deep-reasoning, keep `planning/active/<task-id>/` authoritative, and never treat the companion plan as durable task memory.
