...
function patchedBlock(stepNumber) {
  ...
  '',
  '## Lifecycle Anchor Receipt',
  '',
  'After a real finishing event such as local merge, branch push, or PR creation, record the event as task progress and, when the repository provides it, write a lifecycle anchor receipt under `.harness/lifecycle/anchors/<taskId>/`.',
  '',
  'Anchor receipts are evidence for later `lifecycle-sweep` review. They do not close, block, or archive tasks by themselves.',
  '',
  'A pushed branch is a weak anchor. A created PR usually means `waiting_review`. A verified merge into the recorded base can recommend integration/close review, but final close still belongs to explicit reconcile/close flow.',
  '',
