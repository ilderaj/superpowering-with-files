# Goal2Plan Examples

## Complex Coding Work

Sparse architecture work with unclear repo surfaces:
- audit intake sufficiency and list the missing intake dimensions first
- enrich intake with a bounded native intake audit
- produce one native `/goal` prompt
- use the goal loop to draft and review the implementation plan checkpoint by checkpoint with short `progress.md` updates

## Product Or Operations Work

Broad process or documentation work with multiple stakeholders:
- gather enough success criteria and boundaries
- write a reviewed execution plan without writing implementation artifacts yet
- keep a short checkpoint-based progress log while the plan is being drafted and reviewed
- stop Goal2Plan if the reviewed plan shows direct or tracked execution is already sufficient

## Fallback To Simpler Execution

If the generated plan proves the task is straightforward:
- stop Goal2Plan
- reclassify the work
- continue with direct or tracked execution instead of forcing more planning rounds
