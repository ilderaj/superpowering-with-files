...
Carrier independence has two dimensions: the upstream provider path and the accounting domain. The configured L2 route, `command-code/gpt-5.6-luna`, avoids the `main/gpt-5.6-luna` ChatGPT account route that returned `usage_limit_exceeded` on 2026-09-22 after 6.3s with a reported weekly usage of 100%. The primary had already delivered review candidates, so this was a carrier gap, not a night gap. L2 shares command-code with one L1 target; it is not fully provider-independent. Both carriers also depend on the shared local proxy, machine, network and Host. Command-code has its own availability and quota limits. A model slug and a successful short probe do not authenticate the underlying model or establish equivalent long-running tool performance. Keep provider actual identity unknown without authenticated evidence. This is a bounded recovery path, not a guarantee against every outage.
...
Create a second cron automation against the registered SWF project, on an alternative model route, that runs after the night run's budget has closed. Prompt:

```text
Act as the fallback carrier for this repository's night executor. Your model is an alternative fallback route with shared infrastructure; the primary night pass runs on the DeepSeek combo. Do not duplicate work that already completed.
...
4. Verify the fallback carrier itself. Match Host records to the exact swf-night-executor-fallback automation id, repository and scheduled date in Asia/Shanghai; use actual start/end times to allow scheduler jitter, rather than borrowing another day's latest session. Classify evidenced states as completed-noop, completed-work, running, failed-before-first-model-call, failed-after-start, or not-triggered. Use not-triggered only when a complete Host inventory proves absence after the scheduled time; unreadable or incomplete evidence is unknown. Report a verified failure with its Host error code (for example usage_limit_exceeded), configured model, source and timestamp. Never report a successful no-op as a coverage gap. A fallback failure is a carrier gap, not a night gap by itself. Independently reconcile the primary Host outcome and bound checkpoints: known absence or failure without a recoverable terminal outcome indicates a night execution gap; missing evidence remains unknown. Use the actual startedAt budget and current ownership evidence: an expired cooperative budget does not prove the worker stopped. Never present a missing fallback record as the primary pass having failed. Report only actionable carrier failures or evidence gaps in the coverage-gap line; retain healthy observations locally without duplicate notifications.
