...
- Keep `planning/active/<task-id>/` authoritative, sync durable state after each phase, use `docs/superpowers/plans/<date>-<task-id>.md` plus optional read-only verifier subagents only if a round becomes deep-reasoning, and do not allow goal drift.
