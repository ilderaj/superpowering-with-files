```text
/goal Objective: Stabilize one proof-first goal contract without drifting into unrelated cleanup.
Context: The task is bounded to one workflow-contract surface and can be proven with a short validation loop.
...
Next Step: Re-open the active planning files and patch the workflow-contract surface first.
```
