...
- **Tracked:** create or restore the bound task's three planning files under `planning/active/<task-id>/`: `task_plan.md`, `findings.md`, and `progress.md`. They are the sole durable task authority; no fourth task-state surface. For a new tracked task, load the installed planning-with-files lifecycle reference and initialize from its templates or initializer, preserving the Task ID and Current State block. Check the installed planning-with-files/scripts/planning_paths.py helper and the Host-provided thread ID separately (Codex may expose CODEX_THREAD_ID). For an active task with both available, use bind-thread and bound-task to verify the exact task. Report only the missing prerequisite actually observed; never invent a thread ID or infer binding from a directory name. A waiting or closed task is not an active binding: preserve its lifecycle instead of changing status to satisfy binding.
