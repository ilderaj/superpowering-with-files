...
| New or changed user-facing visuals (a screen, page, dashboard, prototype, demo, mockup, wireframe, or slide/deck layout), or a disputed visual choice | `ux-design`: fix the reader and task, separate fixed constraints from delegated discretion, write observable design decisions and named anti-patterns, and inspect the rendered result at real size; when motion matters, drive, record, and inspect the real flow. Pair with the Host's renderer; when the Host can render in a browser, run the measurement-invariant layer first and then inspect visually. A rendered image is inspection evidence, not acceptance or delivery. |

Route and safety first; resolve domain language before design when needed; settle visual intent before rendering user-visible output; diagnose before a non-obvious fix; use behavior RED/GREEN during implementation and review the completed fixed diff. Repeated failure calls for evidence gathering and reconsidering assumptions, not more speculative patches or an automatic user question after a fixed count. Escalate only an actual missing decision, access, or authorization.
