...
The monitor lifecycle is fail-closed: effective approval requires both PR `reviewDecision=APPROVED` and a current-head `APPROVED` review with authenticated `author.__typename=User` evidence; bot or missing actor type fails closed. `CHANGES_REQUESTED` or `REVIEW_REQUIRED` defeats historical approval. Quietness compares the complete normalized PR/check/review/mergeability/thread snapshot. Review, thread/comment, and check pagination state is independent and monotonic; continue only for a bounded genuinely pending machine check. Stop for `repair_required`, `deferred_follow_up_recording` after deduplicating accepted nonblocking findings into draft issues, `awaiting_human_gate`, `landing_eligibility` with exact-current-head and human-gate evidence, `stale_binding`, `rejected_binding`, `terminal_pr`, or unreadable state.
