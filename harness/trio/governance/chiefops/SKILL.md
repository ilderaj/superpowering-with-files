...
The authority task ID plus frozen `currentSlice` identity reserve the semantic work. The packet digest is immutable evidence and audit binding, never a discriminator that permits a replacement: a revised packet with the same task and slice stays reserved even when its digest changes because declared output or scope changed. A reserved lane without task or current-slice identity is pending, not permission to spawn: an unreleased `awaiting_approval`, `blocked`, or `candidate_done` lane with unknown semantic identity fails closed to `manual_pending:semantic_identity_unbound:<status>` on a non-overlapping spawn, and only an authenticated matching Chief release settles it.
