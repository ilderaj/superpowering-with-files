...
| #197 | Managed Trio projection treated stale recorded ownership as unknown even when destination bytes matched projected source, and the public reconverge result lacked the report shape expected by `syncProduction`. | Fixed with an explicit `sync(['--reconverge'])` path requiring complete manifest ownership, source-byte equivalence only for conflicted managed files, capture-time proof, state readback, and a complete backup. Mixed manual targets report `manual_pending: true`; unknown siblings remain untouched. | `tests/trio/install-upgrade.test.mjs` exercises the public CLI seam with two stale owned files, one older owned methods file, a manual Cursor target, preserved ownership, backup/report fields, normal sync follow-up, custom modification rejection, and symlink rejection. |
...
#197 regression evidence: `tests/trio/install-upgrade.test.mjs` covers two stale global identities equal to new source, one owned older methods file updated by normal sync, preserved ownership entries, preserved unknown sibling, mixed manual Cursor reporting, custom content rejection, and symlink rejection. Full install-upgrade: 64/64; full installer: 161/161.
