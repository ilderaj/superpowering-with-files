...
| New or changed user-facing visuals (a screen, page, dashboard, prototype, demo, mockup, wireframe, or slide/deck layout), or a disputed visual choice | `ux-design`: fix the reader and task, separate fixed constraints from delegated discretion, write observable design decisions and named anti-patterns, and inspect the rendered result at real size; when motion matters, drive, record, and inspect the real flow. Pair with the Host's renderer. A rendered image is inspection evidence, not acceptance or delivery. |
