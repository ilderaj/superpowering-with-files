...
| New or changed user-facing visuals (a screen, page, dashboard, prototype, demo, mockup, wireframe, or slide/deck layout), or a disputed visual choice | `ux-design`: fix the reader and their task, write the observable design decisions and named anti-patterns before building, and inspect the rendered result at real size. Pair with the Host's renderer. A rendered image is inspection evidence, not acceptance or delivery. |
