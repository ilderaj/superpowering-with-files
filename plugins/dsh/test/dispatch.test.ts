...
describe('dispatchWorker execution role gate', () => {
  it('fails closed before start when the packet capability declares no execution work role', async () => {
    await withTmpRoot(async (root) => {
      // Chief slices pass the bind-time capability policy (a Chief role
      // carries no execution role), so dispatch must fail closed before
      // any start attempt.
      ...
      expect(result.status).toBe('manual_pending');
      expect(result.blocker).toBe('execution_role_unavailable');
      expect(harness.subagents.started).toHaveLength(0);
    });
  });

  it('dispatches an execution-role packet without any persona field', async () => {
    await withTmpRoot(async (root) => {
      await boundTask(root, 'role-task');
      const harness = makeMockCtx();
      const dispatcher = createDispatcher(harness.ctx);
      const result = await dispatcher.dispatch({
        authorityRoot: root,
      ...
      expect(result.status).toBe('dispatched');
      expect(harness.subagents.started).toHaveLength(1);
      expect(harness.subagents.started[0]!.request).not.toHaveProperty('persona');
    });
  });
});
