...
| Codex cloud | Research only. | Managed local support does not imply cloud dispatch support. |
| Other cloud environments | Research only. | Use the generic/manual fallback unless a separate cloud contract is verified. |
...
| Repository host boundary | Codex uses the managed `AGENTS.md` and Trio skill surface; this repository does not project hooks. | The retained K3 cloud dispatch is external behavior, not a local host adapter. | Separate boundary. |
