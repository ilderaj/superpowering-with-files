...
2. Before every worktree creation, run `./scripts/harness worktree-preflight --task <task-id> --safety` first, then record the reported base ref and SHA in `planning/active/<task-id>/progress.md`.
3. Use only that command's actual `Suggested worktree label`, `Suggested branch name`, and `Recommended base` to create isolation with `git worktree add <path>/<suggested-label> -b <suggested-branch> <recommended-base>`. Long-running, bypass, and autopilot work belong only in that worktree.
