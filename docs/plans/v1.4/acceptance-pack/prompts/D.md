...
输出仅写入 .harness/acceptance/v14/D/<唯一run-id>/；不得覆盖任何历史 attempt。可在此目录使用临时文件。不得修改源码、计划、测试输入、全局配置或其他任务；不得发邮件、发布或调用生产写入。允许在本任务最终回复中交付给用户。
...
案例输入：docs/plans/v1.4/acceptance-pack/inputs/D-design.md

请按已选定设计交付 preview.html 与 handoff.md，供另一个 Sol/Luna 实现者直接执行。用浏览器实际打开预览，检查四种状态和交互，记录验证结果。
