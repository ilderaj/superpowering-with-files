...
输出仅写入 .harness/acceptance/v14/C/<唯一run-id>/；不得覆盖任何历史 attempt。可在此目录使用临时文件。不得修改源码、计划、测试输入、全局配置或其他任务；不得发邮件、发布或调用生产写入。允许在本任务最终回复中交付给用户。
...
案例输入：docs/plans/v1.4/acceptance-pack/inputs/C-agreement.md

请生成 agreement.docx 清洁配置版，以及 configuration.md 配置指引，让接手人员能对应每一个黄色位置配置三方协议。完成文档结构和实际渲染检查。额外只读核验 tests/fixtures/trio-v2/office/ 下的 document-brief.docx、pdf-review.pdf、presentation-status.pptx、spreadsheet-budget.xlsx 四种既有制品，记录 native parse/render 和表格公式、计算缓存、格式及可见值证据。可复用 scripts/verify-trio-office-artifacts.mjs，先查其用法及可用工具，不要改夹具。
