...
输出仅写入 .harness/acceptance/v14/A/<唯一run-id>/；不得覆盖任何历史 attempt。可在此目录使用临时文件。不得修改源码、计划、测试输入、全局配置或其他任务；不得发邮件、发布或调用生产写入。允许在本任务最终回复中交付给用户。
...
案例输入：docs/plans/v1.4/acceptance-pack/inputs/A-product.md

请给家锐一份可直接评审的中文产品决策包 decision.md：整理当前业务规则、旧计划差异、尚待决定项以及下一步建议，保留来源位置。
