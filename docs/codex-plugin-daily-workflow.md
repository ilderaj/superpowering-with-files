# 日常作业指引（Codex 插件版 Harness）

状态：2026-09-26，本机迁移已完成。适用对象：本机 Codex 用户。范围仅 Codex 和 `~/.codex`、`~/.agents`、SWF 项目，不含 Claude、Cursor 或其他 IDE 的 dot 文件。

## 一分钟版本

平时不用“启动 Harness”。正常说清楚你要什么、边界和已有授权，Host 会自己挑技能；重要任务再用 `$harness-codex-plugin:<技能名>` 点名。开工前只确认一件事：`codex plugin list --json` 里 harness-codex-plugin 是 enabled。需要跨会话续做的工作，明确说“建任务”，落到三文件。

## 现在是什么状态

- 运行入口是私有插件 harness-codex-plugin。本机已装 `2.1.0+codex.20260926`，来自哈希校验过的私有 release 归档。
- 全局 `~/.codex/AGENTS.md` 已从 1096 字节的旧全量 Trio 入口替换成 298 字节的插件短入口；原始字节保存在收据目录 `~/.codex/migrations/swf-plugin-policy-20260926/receipt/`。
- 本仓库 `AGENTS.md` 只有 560 字节，是仓库自有策略，指向 `$harness-codex-plugin:trio`。仓库不再物化 `.agents/skills` 下的 Trio/ChiefOps 投影。
- 迁移前后可量化：本项目 `codex debug prompt-input` 由 32753 字符降到 31938 字符；重复的 `Trio Entry Policy` 由 1 处降到 0 处；技能条目 82 条不变，且没有裸名 `trio`/`chiefops` 与命名空间条目并存。
- 分工固定：插件提供运行时技能正文和辅助脚本；本仓库提供公开安全的规范源码、文档与验收记录；第三方内化技能源码留在私有仓 `ilderaj/swf-harness-codex-plugin`。
- 仍待补的一项：隔离 Container 里新包的真实模型行为矩阵要等测试账号 2026-09-30 配额恢复。本机 cutover 本身已验收。

## 开工前只做一件事：确认插件在

    codex plugin list --json

期望看到 `harness-codex-plugin@personal`、`enabled=true`、版本 `2.1.0+codex.20260926`（或同一身份的更新版本）。没有就按 `docs/install/codex-plugin-private.md` 安装或升级。安装不附带 MCP、凭据、模型权限或项目规则，这些仍然是各自独立的一步。
...
全局 `~/.codex/AGENTS.md` 和项目 `AGENTS.md` 是两件独立的事。本仓库的 `AGENTS.md` 归仓库自己所有，`project.mjs status` 会报 `owner=repository`，不会被改写。
...
本机已经迁移完成。要回滚本机：
    ...
    node <插件根>/scripts/policy-migration.mjs rollback --receipt ~/.codex/migrations/swf-plugin-policy-20260926/receipt/receipt.json

要在一台仍带旧全量 Trio 全局入口的机器上做同样迁移（收据目录必须尚不存在）：

    node <插件根>/scripts/policy-migration.mjs plan --global ~/.codex/AGENTS.md --receipt-dir <新目录>/receipt > <新目录>/plan.json
    node <插件根>/scripts/policy-migration.mjs apply --plan <新目录>/plan.json
    node <插件根>/scripts/policy-migration.mjs status --receipt <新目录>/receipt/receipt.json
