...
Each companion carries the full official Matt Pocock catalog — 24 skills (17 engineering, 7 productivity) locked to upstream `mattpocock/skills` v1.2.3 — as full directory copies, plus its host manifest, `LICENSE`, `UPSTREAM.json`, and `OVERLAYS.json`. Every skill is independently toggleable in the host, so the catalog is opt-in at the skill level. `grill-me` and `grilling` are explicit opt-in skills. `to-questionnaire` creates a local Markdown draft; external delivery remains human-gated.

The native companion uses its own `.codex-plugin/plugin.json` marketplace package. The portable companion has a root `plugin.json` and 24 flat skill directories for non-recursive Agent Plugins discovery. It remains client-owned: follow the compatible client's own procedure and do not claim a remote install.
...
For the opt-in companions, confirm the native `.codex-plugin/plugin.json` or portable root `plugin.json`, `LICENSE`, `UPSTREAM.json`, and `OVERLAYS.json`, and that all 24 `skills/<name>/SKILL.md` files (with their extra assets such as `agents/` and `scripts/`) are present. The companion remains separate from Trio: `grill-me` and `grilling` are explicit opt-in, `to-questionnaire` creates a local Markdown draft, and external delivery remains human-gated. The portable package follows the client's own procedure; if a client CLI is unavailable, record that limitation and do not claim host-level installation verification.
