...
The release note should name the two core artifacts — the native Codex Trio package and portable Agent Plugins v1 package — and the two independent, opt-in Matt companion artifacts covering the full official Matt catalog (24 skills, independently toggleable). The companion packages make no change to Trio or its projection. `grill-me` and `grilling` are explicit opt-in; `to-questionnaire` creates a local Markdown draft; external delivery remains human-gated.
