...
Each release also ships a portable Agent Plugins v1 package (`harness-agent-plugins-<version>.tgz`) for clients that implement the standard. It contains the same eight skills as flat immediate children plus a root `plugin.json`, and is installed manually per the client's own procedure — this repository performs no managed generic-client installation. See [Agent Plugins installation](agent-plugins.md).

Each release also includes independent, opt-in Matt companion archives: `harness-matt-skills-codex-plugin-<version>.tgz` and `harness-matt-skills-agent-plugins-<version>.tgz`. They make no change to Trio or its projection and are not another Trio target. They contain the full official Matt catalog — 24 independently toggleable skills (17 engineering, 7 productivity) — locked to upstream v1.2.3: `grill-me` and `grilling` are explicit opt-in, `to-questionnaire` creates a local Markdown draft, and external delivery remains human-gated. Verify the archive against `SHA256SUMS` and `manifest.json`. The Codex companion has a separate marketplace root and manifest; the portable companion is client-owned and follows the client's own procedure, with no remote install claim.
