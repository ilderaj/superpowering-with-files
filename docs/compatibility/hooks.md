...
Goal-loop discipline does not depend on hooks. The Goal Round Start Protocol for `/goal`, `/plan-goal`, and similar continuation flows lives in rendered policy and skill guidance. Hooks only inject compact reminders to reopen the authoritative planning files and resume from the current task state.
