...
| The work produces or changes something a person looks at (a screen, page, dashboard, prototype, demo, mockup, wireframe, or slide/deck layout), or a visual design choice is disputed. | `ux-design` | State the reader and task, fixed constraints, delegated discretion, observable decisions, and named anti-patterns before building; inspect the rendered output at real size and, when motion matters, record and inspect successive frames. | Publish, deploy, share, or claim acceptance; turn an unaccepted proposal into a fixed requirement; treat a generated file or design export as user-visible delivery; invent brand assets, copy, or metrics. |

Priority: route and safety first; domain modeling before design when terms affect the interface; diagnosis before a non-obvious bug fix; TDD inside every behavior-changing slice; visual intent before rendering user-visible output; review after the fixed diff is complete. Record the selected methods and their evidence in the existing Trio. If a trigger reveals material ambiguity, broad impact, or a required human gate, escalate to Chief before production mutation.
