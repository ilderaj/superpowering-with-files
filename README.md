...
The plugin is now the runtime entry for this Harness. The public repository no longer materializes the project-local Trio/ChiefOps projection, and both `dev` and `main` carry the short `AGENTS.md` entry that routes into `$harness-codex-plugin:trio`. A candidate `v2.1.0-rc.1+codex.20260926` adds reversible `AGENTS.md` policy migration and a repository-aware project opt-in; it is published as a private pre-release and is not yet the installed Host version. See the [daily workflow guide](docs/codex-plugin-daily-workflow.md) for intake, explicit skill invocation, project opt-in, upgrade, and rollback.
